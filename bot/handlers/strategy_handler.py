...
async def handle_strategy_description_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Get strategy data
    strategy_data = await state_manager.get_state_data(user.id)
    strategy_type = strategy_data.get('strategy_type', 'straddle')
    name = strategy_data.get('name', 'Unknown')
    
    # Store description (or skip if /skip)
    ...
    await state_manager.set_state(user.id, 'strategy_add_asset')
    ...
    log_user_action(user.id, "strategy_description", f"Added description for {name}")
