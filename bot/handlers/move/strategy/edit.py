"""
MOVE Strategy Edit Handler - CALLBACK ONLY

Handles editing existing MOVE strategies.
All text input routing goes through input_handlers.py
"""

from telegram import Update
from telegram.error import BadRequest  # ✅ CORRECT (v22.5+)
from telegram.ext import ContextTypes, CallbackQueryHandler, Application

from bot.utils.logger import setup_logger, log_user_action
from bot.utils.error_handler import error_handler
from bot.utils.state_manager import state_manager
from bot.validators.user_validator import check_user_authorization
from database.operations.move_strategy_ops import (
    get_move_strategies,
    get_move_strategy,
    update_move_strategy
)
from bot.keyboards.move_strategy_keyboards import (
    get_strategy_list_keyboard,
    get_edit_fields_keyboard,
    get_move_menu_keyboard,
    get_cancel_keyboard,
    get_continue_edit_keyboard,
    get_edit_asset_keyboard,
    get_edit_expiry_keyboard,
    get_edit_direction_keyboard
)

logger = setup_logger(__name__)

@error_handler
async def move_edit_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show list of MOVE strategies to edit."""
    query = update.callback_query
    user = query.from_user
    
    logger.info("=" * 60)
    logger.info("🔵 MOVE EDIT CALLBACK - TRIGGERED")
    logger.info(f"   User: {user.id} ({user.first_name})")
    logger.info(f"   Callback Data: {query.data}")
    
    await query.answer()
    
    if not await check_user_authorization(user):
        logger.warning(f"   ❌ User {user.id} not authorized for MOVE edit")
        await query.edit_message_text("❌ Unauthorized access.")
        return
    
    log_user_action(user.id, "move_edit", "Requested MOVE strategy list for editing")
    
    try:
        strategies = await get_move_strategies(user.id)
        logger.info(f"   ✓ Retrieved {len(strategies) if strategies else 0} strategies")
        
        if not strategies:
            logger.info(f"   ℹ️ No strategies found for user {user.id}")
            await query.edit_message_text(
                "📋 No MOVE strategies found.\n\n"
                "Create your first strategy!",
                reply_markup=get_move_menu_keyboard(),
                parse_mode='HTML'
            )
            return
        
        logger.info(f"   ✓ Showing edit menu with {len(strategies)} strategies")
        await query.edit_message_text(
            "📝 Edit MOVE Strategy\n\n"
            "Select a strategy to edit:",
            reply_markup=get_strategy_list_keyboard(strategies, action='edit'),
            parse_mode='HTML'
        )
        logger.info("   ✅ Successfully sent strategy list keyboard")
    except Exception as e:
        logger.error(f"   ❌ Error in move_edit_callback: {str(e)}", exc_info=True)
        raise
    finally:
        logger.info("=" * 60)

@error_handler
async def move_edit_select_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle strategy selection for editing.
    First call shows list, second call with move_edit_{ID} shows details.
    """
    query = update.callback_query
    user = query.from_user
    
    logger.info("=" * 60)
    logger.info("🟢 MOVE EDIT SELECT CALLBACK - TRIGGERED")
    logger.info(f"   User: {user.id}")
    logger.info(f"   Callback Data: {query.data}")
    
    await query.answer()
    
    try:
        # CASE 1: Initial move_edit call (show list)
        if query.data == "move_edit":
            logger.info("   ℹ️ CASE 1: Initial move_edit call - delegating to move_edit_callback")
            return await move_edit_callback(update, context)
        
        # CASE 2: move_edit_{strategy_id} call (select specific strategy)
        logger.info("   ℹ️ CASE 2: Strategy selection detected")
        parts = query.data.split('_')
        strategy_id = parts[2] if len(parts) >= 3 else None
        
        logger.info(f"   Parsed: parts={parts}, strategy_id={strategy_id}")
        
        if not strategy_id:
            logger.warning(f"   ❌ Invalid strategy_id extracted")
            await query.edit_message_text(
                "❌ Invalid request.",
                reply_markup=get_move_menu_keyboard(),
                parse_mode='HTML'
            )
            return
        
        logger.info(f"   ✓ Fetching strategy {strategy_id}")
        strategy = await get_move_strategy(user.id, strategy_id)
        
        if not strategy:
            logger.warning(f"   ❌ Strategy {strategy_id} not found for user {user.id}")
            await query.edit_message_text(
                "❌ Strategy not found.",
                reply_markup=get_move_menu_keyboard(),
                parse_mode='HTML'
            )
            return
        
        logger.info(f"   ✓ Strategy found: {strategy.get('strategy_name')}")
        
        await state_manager.set_state_data(user.id, {
            'editing_strategy_id': strategy_id,
            'strategy_data': strategy
        })
        logger.info(f"   ✓ State data saved for user {user.id}")
        
        keyboard = get_edit_fields_keyboard(strategy_id)
        logger.info(f"   ✓ Edit fields keyboard generated")
        
        try:
            await query.edit_message_text(
                f"📝 Edit Strategy: {strategy.get('strategy_name')}\n\n"
                f"Current Settings:\n"
                f"• Asset: {strategy.get('asset')}\n"
                f"• Expiry: {strategy.get('expiry', 'daily').capitalize()}\n"
                f"• Direction: {strategy.get('direction', 'N/A').capitalize()}\n"
                f"• ATM Offset: {strategy.get('atm_offset', 0)}\n\n"
                f"What would you like to edit?",
                reply_markup=keyboard,
                parse_mode='HTML'
            )
            logger.info("   ✅ Successfully sent edit menu")
        except BadRequest as e:
            if "message is not modified" in str(e).lower():
                logger.warning("   ⚠️  Message not modified (probably same content)")
            else:
                logger.error(f"   ❌ BadRequest error: {str(e)}", exc_info=True)
                raise
    except Exception as e:
        logger.error(f"   ❌ Error in move_edit_select_callback: {str(e)}", exc_info=True)
        raise
    finally:
        logger.info("=" * 60)

@error_handler
async def move_edit_field_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handle field selection for editing.
    Callback format: move_edit_field_{strategy_id}_{field_name}
    """
    query = update.callback_query
    user = query.from_user
    
    logger.info("=" * 60)
    logger.info("🟡 MOVE EDIT FIELD CALLBACK - TRIGGERED")
    logger.info(f"   User: {user.id}")
    logger.info(f"   Callback Data: {query.data}")
    
    await query.answer()
    
    try:
        # Extract: move_edit_field_{ID}_{field} -> ['move', 'edit', 'field', 'ID', 'field_name']
        parts = query.data.split('_', 4)  # Limit splits to preserve field names
        strategy_id = parts[3] if len(parts) >= 4 else None
        field = parts[4] if len(parts) >= 5 else None
        
        logger.info(f"   Parsed: parts={parts}")
        logger.info(f"   Strategy ID: {strategy_id}, Field: {field}")
        
        if not strategy_id or not field:
            logger.warning(f"   ❌ Missing strategy_id or field")
            await query.edit_message_text(
                "❌ Invalid request.",
                reply_markup=get_move_menu_keyboard()
            )
            return
        
        await state_manager.set_state_data(user.id, {
            'editing_field': field,
            'editing_strategy_id': strategy_id
        })
        logger.info(f"   ✓ State set: editing_field={field}, strategy_id={strategy_id}")
        
        data = await state_manager.get_state_data(user.id)
        strategy = data.get('strategy_data', {})
        logger.info(f"   ✓ Retrieved strategy data from state")
        
        if field == 'name':
            logger.info(f"   ℹ️ Editing NAME field")
            await state_manager.set_state(user.id, 'move_edit_name')
            await query.edit_message_text(
                f"📝 Edit Strategy Name\n\n"
                f"Current: {strategy.get('strategy_name')}\n\n"
                f"Enter new name:",
                reply_markup=get_cancel_keyboard(),
                parse_mode='HTML'
            )
        
        elif field == 'description':
            logger.info(f"   ℹ️ Editing DESCRIPTION field")
            await state_manager.set_state(user.id, 'move_edit_description')
            await query.edit_message_text(
                f"📝 Edit Description\n\n"
                f"Current: {strategy.get('description', 'None')}\n\n"
                f"Enter new description:",
                reply_markup=get_cancel_keyboard(),
                parse_mode='HTML'
            )
        
        elif field == 'asset':
            logger.info(f"   ℹ️ Editing ASSET field")
            await query.edit_message_text(
                f"📝 Edit Asset\n\n"
                f"Current: {strategy.get('asset')}\n\n"
                f"Select new asset:",
                reply_markup=get_edit_asset_keyboard(strategy_id),
                parse_mode='HTML'
            )
        
        elif field == 'expiry':
            logger.info(f"   ℹ️ Editing EXPIRY field")
            await query.edit_message_text(
                f"📝 Edit Expiry\n\n"
                f"Current: {strategy.get('expiry', 'daily').capitalize()}\n\n"
                f"Select new expiry:",
                reply_markup=get_edit_expiry_keyboard(strategy_id),
                parse_mode='HTML'
            )
        
        elif field == 'direction':
            logger.info(f"   ℹ️ Editing DIRECTION field")
            await query.edit_message_text(
                f"📝 Edit Direction\n\n"
                f"Current: {strategy.get('direction', 'N/A').capitalize()}\n\n"
                f"Select new direction:",
                reply_markup=get_edit_direction_keyboard(strategy_id),
                parse_mode='HTML'
            )
        
        elif field == 'atm_offset':
            logger.info(f"   ℹ️ Editing ATM_OFFSET field")
            await state_manager.set_state(user.id, 'move_edit_atm_offset')
            await query.edit_message_text(
                f"📝 Edit ATM Offset\n\n"
                f"Current: {strategy.get('atm_offset', 0)}\n\n"
                f"Enter new ATM offset (-10 to +10):",
                reply_markup=get_cancel_keyboard(),
                parse_mode='HTML'
            )
        
        elif field in ['sl_trigger', 'sl_limit', 'target_trigger', 'target_limit']:
            logger.info(f"   ℹ️ Editing RISK MANAGEMENT field: {field}")
            await state_manager.set_state(user.id, f'move_edit_{field}')
            field_display = field.replace('_', ' ').title()
            await query.edit_message_text(
                f"📝 Edit {field_display}\n\n"
                f"Current: {strategy.get(field, 'Not set')}\n\n"
                f"Enter new value (percentage):",
                reply_markup=get_cancel_keyboard(),
                parse_mode='HTML'
            )
        else:
            logger.warning(f"   ⚠️ Unknown field: {field}")
        
        logger.info("   ✅ Field edit menu sent successfully")
    except Exception as e:
        logger.error(f"   ❌ Error in move_edit_field_callback: {str(e)}", exc_info=True)
        raise
    finally:
        logger.info("=" * 60)

@error_handler
async def move_edit_save_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Save callback-based edits (asset, expiry, direction).
    Format: move_edit_save_{field}_{strategy_id}_{value}
    """
    query = update.callback_query
    user = query.from_user
    
    logger.info("=" * 60)
    logger.info("🟠 MOVE EDIT SAVE CALLBACK - TRIGGERED")
    logger.info(f"   User: {user.id}")
    logger.info(f"   Callback Data: {query.data}")
    
    await query.answer()
    
    try:
        # Extract: move_edit_save_asset_ID_BTC -> ['move', 'edit', 'save', 'asset', 'ID', 'BTC']
        parts = query.data.split('_')
        field = parts[3] if len(parts) >= 4 else None
        strategy_id = parts[4] if len(parts) >= 5 else None
        new_value = parts[5] if len(parts) >= 6 else None
        
        logger.info(f"   Parsed: field={field}, strategy_id={strategy_id}, value={new_value}")
        
        if not all([field, strategy_id, new_value]):
            logger.warning(f"   ❌ Missing required parts")
            await query.edit_message_text(
                "❌ Invalid request.",
                reply_markup=get_move_menu_keyboard()
            )
            return
        
        # Map field to database column
        field_mapping = {
            'asset': 'asset',
            'expiry': 'expiry',
            'direction': 'direction'
        }
        
        db_field = field_mapping.get(field)
        logger.info(f"   Field mapping: {field} -> {db_field}")
        
        if not db_field:
            logger.warning(f"   ❌ Field '{field}' not in mapping")
            await query.edit_message_text(
                "❌ Invalid field.",
                reply_markup=get_move_menu_keyboard()
            )
            return
        
        # Update strategy
        update_data = {db_field: new_value}
        logger.info(f"   Updating database: {update_data}")
        result = await update_move_strategy(user.id, strategy_id, update_data)
        
        if result:
            logger.info(f"   ✅ Update successful")
            log_user_action(user.id, f"move_edit_save_{field}", f"Updated {field}: {new_value}")
            
            await query.edit_message_text(
                f"✅ Strategy Updated!\n\n"
                f"{field.capitalize()} changed to: {new_value.capitalize()}\n\n"
                f"Continue editing or return to menu?",
                reply_markup=get_continue_edit_keyboard(strategy_id),
                parse_mode='HTML'
            )
        else:
            logger.warning(f"   ❌ Update failed for strategy {strategy_id}")
            await query.edit_message_text(
                "❌ Failed to update strategy.",
                reply_markup=get_move_menu_keyboard()
            )
    except Exception as e:
        logger.error(f"   ❌ Error in move_edit_save_callback: {str(e)}", exc_info=True)
        raise
    finally:
        logger.info("=" * 60)


# ✅ REGISTRATION FUNCTION - CALLBACK ONLY (NO TEXT HANDLER)
def register_move_edit_handlers(app: Application):
    """
    Register MOVE strategy edit handlers (callbacks only).
    
    ✅ KEY: Text input is handled by input_handlers.py route_move_message()
    NOT here! This prevents handler conflicts in Group 11.
    """
    
    logger.info("📋 Registering MOVE edit handlers...")
    
    # Most specific FIRST
    app.add_handler(
        CallbackQueryHandler(move_edit_callback, pattern="^move_edit$"),
        group=10
    )
    logger.info("   ✓ Handler 1: move_edit (exact match)")
    
    app.add_handler(
        CallbackQueryHandler(move_edit_save_callback, pattern="^move_edit_save_"),
        group=10
    )
    logger.info("   ✓ Handler 2: move_edit_save_*")
    
    app.add_handler(
        CallbackQueryHandler(move_edit_field_callback, pattern="^move_edit_field_"),
        group=10
    )
    logger.info("   ✓ Handler 3: move_edit_field_*")
    
    # Most generic LAST
    app.add_handler(
        CallbackQueryHandler(move_edit_select_callback, pattern="^move_edit_[0-9a-f]{24}$"),
        group=10
    )
    logger.info("   ✓ Handler 4: move_edit_[24-char-id]")
    
    # ✅ NO MESSAGE HANDLER REGISTERED HERE!
    # Text input routing is handled by input_handlers.py route_move_message()
    
    logger.info("✅ MOVE edit handlers (callback only) registered successfully!")


__all__ = [
    'move_edit_callback',
    'move_edit_select_callback',
    'move_edit_field_callback',
    'move_edit_save_callback',
    'register_move_edit_handlers',
        ]
